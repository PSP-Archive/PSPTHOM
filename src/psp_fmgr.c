...
# if 0
        if (user_file_format == FMGR_FORMAT_STATE) /* State */ error = thom_load_state(user_filename, 1);
        else
...
# endif
      ...
      else 
      {
        ...
        if (file_format == FMGR_FORMAT_CHT) /* Cheat */  error = thom_load_file_cheat(user_filename);
      }
    }
