...
# define MENU_SET_VSYNC         7
# define MENU_SET_ANALOG        8
# define MENU_SET_AUTOFIRE_T    9
# define MENU_SET_AUTOFIRE_M   10
# define MENU_SET_DISPLAY_LR   11
# define MENU_SET_CLOCK        12
# define MENU_SET_BASIC        13
...
# define MAX_MENU_SET_ITEM (MENU_SET_BACK + 1)
  ...
  static int thom_vsync            = 0;
  static int thom_render_mode      = 0;
  static int thom_delta_y          = 0;
  ...
  static int thom_auto_fire_mode   = 0;
...
psp_display_screen_settings_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_SET_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SET_AUTOFIRE_M) {
      if (thom_auto_fire_mode) strcpy(buffer,"yes");
      else                    strcpy(buffer,"no ");
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
    ...
    if (menu_id == MENU_SET_VSYNC) {
      if (thom_vsync) strcpy(buffer,"yes");
      else                strcpy(buffer,"no ");
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
    if (menu_id == MENU_SET_RENDER) {
      ...
      if (thom_render_mode == THOM_RENDER_NORMAL) strcpy(buffer, "normal");
      else
      if (thom_render_mode == THOM_RENDER_X125  ) strcpy(buffer, "x1.25");
      else
      if (thom_render_mode == THOM_RENDER_X15   ) strcpy(buffer, "x1.5");
      else
      if (thom_render_mode == THOM_RENDER_X175  ) strcpy(buffer, "x1.75");
      else
      if (thom_render_mode == THOM_RENDER_FIT   ) strcpy(buffer, "fit");
      else                                        strcpy(buffer, "height");

      string_fill_with_space(buffer, 13);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
    if (menu_id == MENU_SET_DELTA_Y) {
      sprintf(buffer,"%d", thom_delta_y);
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
...
{
  if (step > 0) {
    if (thom_delta_y < 20) thom_delta_y++;
  } else {
    if (thom_delta_y >  -20) thom_delta_y--;
  }
}
...
psp_settings_menu_init(void)
{
  ...
  thom_delta_y          = THOM.thom_delta_y;
  thom_vsync            = THOM.thom_vsync;
...
psp_settings_menu_validate(void)
{
  ...
  THOM.thom_delta_y       = thom_delta_y;
  THOM.thom_vsync          = thom_vsync;
  THOM.psp_cpu_clock       = psp_cpu_clock;
  THOM.thom_speed_limiter  = thom_speed_limiter;

  THOM.psp_reverse_analog  = psp_reverse_analog;
  ...
  if (thom_auto_fire_mode != THOM.thom_auto_fire) {
    kbd_change_auto_fire(thom_auto_fire_mode);
  }
...
# endif
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        break;              
        case MENU_SET_VSYNC      : thom_vsync = ! thom_vsync;
        break;              
        case MENU_SET_AUTOFIRE_T  : psp_settings_menu_autofire( step );
        break;              
        case MENU_SET_AUTOFIRE_M  : thom_auto_fire_mode = ! thom_auto_fire_mode;
        break;              
