...
# define MENU_LOAD_DISK0   7
# define MENU_LOAD_DISK1   8
...
psp_menu_display_save_name()
{
  ...
  snprintf(buffer, 20, "Game: %s", THOM.thom_save_name);
...
psp_display_screen_menu(void)
{
  ...
  for (slot_id = 0; slot_id < THOM_MAX_SAVE_STATE; slot_id++) {
    ...
    if (THOM.thom_save_state[slot_id].used) {
      ScePspDateTime* my_date = &THOM.thom_save_state[slot_id].date;
# ifndef LINUX_MODE
...
# endif
  ...
  if (THOM.thom_save_state[cur_slot].thumb) {
    psp_sdl_blit_thumb(280, 50, THOM.thom_save_state[cur_slot].surface);
  } else {
...
psp_main_menu_cold_reset(void)
{
  ...
  psp_sdl_back2_print(140, 160, "Cold Reset TO7-70 !", 
...
psp_main_menu_soft_reset(void)
{
  /* Reset ! */
  psp_display_screen_menu();
  psp_sdl_back2_print(140, 160, "Soft Reset TO7-70 !", PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
  emulator_soft_reset();
  reset_save_name();
...
psp_main_menu_editor()
{
  ...
  sprintf(TmpFileName, "%s/txt/%s.txt", THOM.thom_home_dir, THOM.thom_save_name);
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        case MENU_LOAD_K7    : psp_menu_dirty = 1;
                               if (psp_main_menu_load(FMGR_FORMAT_K7, 0)) end_menu = 1;
                               old_pad = new_pad = 0;
                               psp_menu_dirty = 1;
        break;              
        ...
        case MENU_LOAD_DISK0 : psp_menu_dirty = 1;
                               if (psp_main_menu_load(FMGR_FORMAT_SAP, 0)) end_menu = 1;
                               old_pad = new_pad = 0;
                               psp_menu_dirty = 1;
        break;              
        case MENU_LOAD_DISK1 : psp_menu_dirty = 1;
                               if (psp_main_menu_load(FMGR_FORMAT_SAP, 1)) end_menu = 1;
                               old_pad = new_pad = 0;
                               psp_menu_dirty = 1;
        break;              
        case MENU_LOAD_STATE : psp_menu_dirty = 1;
                               if (psp_main_menu_load(FMGR_FORMAT_STATE, 0)) end_menu = 1;
                               old_pad = new_pad = 0;
                               psp_menu_dirty = 1;
        break;              
